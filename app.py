...
def derive_status(row, risk_c, label_c, thresh):
    if label_c is not None:
        ...
        if raw in {"1", "1.0", "TRUE", "CONFIRMED", "YES"}:
            return "Confirmed"
        if raw in {"FLAGGED", "ELEVATED", "HIGH RISK"}:
            return "Flagged"
    ...
    with right:
        ...
        for _, r in board.iterrows():
            ...
            if C_CO2YOY and pd.notna(r.get(C_CO2YOY)) and C_SPEC and pd.notna(r.get(C_SPEC)):
                ...
                dchip = (
                    f'<div class="dchip">CO\u2082 <b class="{co2_cls}">{co2_arrow} '
                    f"{co2:+.1f}%</b> &nbsp;\u00b7&nbsp; Spec "
                    f"<b>{float(r[C_SPEC]):.3f}</b></div>"
                )
            st.markdown(
                f"""<div class="lrow">
                <div style="display:flex;justify-content:space-between;align-items:baseline">
                  <span><span class="name">{r[C_COMP]}</span>
                  <span class="yr">{r[C_YEAR]}</span></span>
                  <span class="chip" style="color:{fg};background:{bg}">{status}</span>
                </div>
                <div style="display:flex;justify-content:space-between;margin-top:4px">
                  <span class="score">{risk:.3f}</span>
                </div>
                <div class="riskbar"><div style="width:{bar:.0f}%;background:{fg}"></div></div>
                {dchip}
                </div>""",
                unsafe_allow_html=True,
            )
